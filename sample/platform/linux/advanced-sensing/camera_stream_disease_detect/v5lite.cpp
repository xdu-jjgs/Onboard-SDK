...
bool V5lite::InferenceImage(cv::Mat &img) {
    busy = true;
    //get context
    assert(engine != nullptr);
    ...
    EngineInference(img, outSize, buffers, bufferSize, stream);
...
void V5lite::EngineInference(cv::Mat &img, const int &outSize, void **buffers,
    ...
    std::cout << "Processing" << " test.jpg" << std::endl;
    ...
    cv::cvtColor(org_img, org_img, cv::COLOR_RGB2BGR);
